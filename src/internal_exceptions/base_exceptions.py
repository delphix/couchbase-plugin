...
##############################################################################
"""
We are defining two base classes for two types of exceptions: one is related
to database & the other one is for run-time errors in the plugin. Both classes
are child class of Exception which is defined inside python
The purpose of segregation of these two kinds of exceptions is to get a more
accurate message at runtime error.
All the exceptions created for the database will inherit the DatabaseException
and these are defined in the current package.
"""
##############################################################################
...
# Exceptions related to plugin operation like discovery, linking,
# virtualization are being handled using this.
# plugin_exceptions.py is responsible to catch and throw specific error
# message for each kind of delphix operation.
